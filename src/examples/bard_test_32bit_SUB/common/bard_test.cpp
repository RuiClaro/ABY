...
int32_t test_bard_test_circuit(e_role role, char* address, uint16_t port, seclvl seclvl,
	...
	ofstream validation("validation_32_SUB.txt",ios::app);
	ofstream results("results_32_SUB.txt",ios::app);
	...
	for (int i = (-pow(2,31)+1); i < pow(2,31); i+=42953)
	//for (int i = 244; i <= 244; i +=100)
	{
		//A->244;B->-8999
	//	for (int j = -8999; j <= -8999; j +=100)
		for (int j = (-pow(2,31)+1); j < pow(2,31); j+=42943)
		{
			
			int32_t a = i;
			int32_t b = j;

			if(a-b >= pow(2,31) || a-b <= -pow(2,31)){
				/*cout << "a=" << a << "\n";
				cout << "b=" << b << "\n";*/
				continue;
			}
