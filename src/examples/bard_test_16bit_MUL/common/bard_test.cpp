...
int32_t test_bard_test_circuit(e_role role, char* address, uint16_t port, seclvl seclvl,
	...
	ofstream validation("validation_16_MUL.txt",ios::app);
	ofstream results("results_16_MUL.txt",ios::app);
	...
	for (int i = (-pow(2,15)+1); i < pow(2,15); i+=29)
	//for (int i = 244; i <= 244; i +=100)
	{
		//A->244;B->-8999
	//	for (int j = -8999; j <= -8999; j +=100)
		for (int j = (-pow(2,15)+1); j < pow(2,15); j+=23)
		{
...
share* BuildBardTestCircuit(share *s_alice, share *s_bob,
	...
	out = bc->PutMULGate(s_alice, s_bob);
