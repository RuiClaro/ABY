...
int32_t test_bard_test_circuit(e_role role, char* address, uint16_t port, seclvl seclvl,
	...
	ofstream validation("validation_16_SUB.txt",ios::app);
	ofstream results("results_16_SUB.txt",ios::app);
	...
	for (int i = (-pow(2,15)+1); i < pow(2,15); i+=3001)
	//for (int i = 244; i <= 244; i +=100)
	{
		//A->244;B->-8999
	//	for (int j = -8999; j <= -8999; j +=100)
		for (int j = (-pow(2,15)+1); j < pow(2,15); j+=2971)
		{
			
			int16_t a = i;
			int16_t b = j;

			if(a-b >= pow(2,15) || a-b <= -pow(2,15)){
				cout << "a=" << a << "\n";
				cout << "b=" << b << "\n";
				continue;
			}
			...
			*/
			...
			uint16_t alice_money, bob_money, output;
			...
			*/
			...
			if(role_id == 0) {
				...
				results << "SUB->" << int(outp) << endl;
			} 
			
			cout << "\nTesting " << get_sharing_name(sharing)
						<< " sharing: " << endl;
			cout << "\nAlice :\t" << alice_money;
			cout << "\nBob :\t" << bob_money;
			cout << "\nresult:\t" << outp;
...
share* BuildBardTestCircuit(share *s_alice, share *s_bob,
	...
	out = bc->PutSUBGate(s_alice, s_bob);
