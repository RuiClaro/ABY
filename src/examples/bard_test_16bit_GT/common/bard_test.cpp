...
int32_t test_bard_test_circuit(e_role role, char* address, uint16_t port, seclvl seclvl,
	...
	ofstream validation("validation_16_GT.txt",ios::app);
	ofstream results("results_16_GT.txt",ios::app);
	...
	//	for (int j = -8999; j <= -8999; j +=100)
		for (int j = (-pow(2,15)+1); j < pow(2,15); j+=2971)
		{
			...
			if(role_id == 0) {
				...
				if(a > b){
					validation << "GT->1" << endl;		
				}else{
					validation << "GT->0" << endl;
			...
			*/
			...
			//cout << "\nBefore Computation\t";
			alice_money = pow(2,15) + a;
			bob_money = pow(2,15) + b;
